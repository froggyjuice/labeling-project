# Google Drive 이미지 라벨링 서비스 - 전문가 리뷰용

## 🎯 프로젝트 목적

### **서비스 개요**
Google Drive에 저장된 이미지들을 웹 인터페이스를 통해 라벨링(분류)할 수 있는 협업 플랫폼입니다. 
...
### 문제 1: 로딩 속도 느림
- **증상**: 로그인/대시보드 페이지 로딩이 5-10초 걸림 (늘 그런건 아닌데, 간만에 들어가면...?)
- **원인**: DB 쿼리 최적화 부족, 캐싱 미흡
- **테스트**: http://localhost:8000/login/ 접속 후 로딩 시간 측정
...
**실행 중 문제 발생 시**: `docker-compose -f docker-compose.dev.yml logs web` 로그 확인 후 문의 
